...
public class SparseMyceliumSlabBlock extends CustomSlabBlock {
    private static VoxelShape BOTTOM_SHAPE;
    private static VoxelShape FULL_SHAPE;
    public SparseMyceliumSlabBlock(Settings settings) {
        super(settings);
    }
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        if (!canSurvive(state, world, pos)) {
            ...
            if (random.nextInt(25) == 0) {
                world.setBlockState(pos, (BlockState)slabBlockState.with(TYPE, world.getBlockState(pos).get(TYPE)).with(SNOWY, world.getBlockState(pos.up()).isOf(Blocks.SNOW) && world.getBlockState(pos).get(TYPE) == SlabType.DOUBLE));
            }
        }
    }
