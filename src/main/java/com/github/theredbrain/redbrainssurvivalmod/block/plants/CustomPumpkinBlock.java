...
public class CustomPumpkinBlock extends FallingGourdBlock {
    public CustomPumpkinBlock(Settings settings) {
        super(settings);
    }

    @Override
    public CustomStemBlock getStem() {
        return (CustomStemBlock) BlocksRegistry.PUMPKIN_STEM.get();
    }

    @Override
    public CustomAttachedStemBlock getAttachedStem() {
        return (CustomAttachedStemBlock) BlocksRegistry.ATTACHED_PUMPKIN_STEM.get();
    }
